...
secondsModLeftWhenPaused () { echo $(( $(secondsLeftWhenPaused) % 60 )) ; }
secondsLeft () { echo $(( $(timerExpiry) - $(now) )) ; }
minutesLeft () { echo $(( $(secondsLeft) / 60 )) ; }
secondsModLeft () { echo $(( $(secondsLeft) % 60 )) ; }
...
updateTail () {
  ...
  if timerSet
  then
    if { timerPaused && [ $(minutesLeftWhenPaused) -le 0 ] ; } || { ! timerPaused && [ $(secondsLeft) -le 0 ] ; }
    then
      eval $(timerAction)
      killTimer
      removePrinting
  ...
  then
    if timerPaused
    then
      echo "$(timerLabelPaused) $(minutesLeftWhenPaused) $(secondsModLeftWhenPaused)"
    else
      echo "$(timerLabelRunning) $(minutesLeft) $(secondsModLeft)"
    fi
  else
    echo "${STANDBY_LABEL}"
  fi
